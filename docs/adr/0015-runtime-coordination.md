...
// and non-reversible — ObserveLock never exposes release capability.
...
type LockForcer interface {
	// ObserveLock reports the current holder of key, if held. Because Identity
	// is the universal SHA-256 derivation, any caller can recognize a holder it
	// installed via TakeLock by hashing its own replacement token — no
	// backend-specific comparison is needed.
