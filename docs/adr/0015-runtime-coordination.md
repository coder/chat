...
- Three new **Runtime Options** (`MaxDetached`, optional `MaxDetachedPerTenant`, `MaxBurstBatch`), one new sentinel (`ErrAdmissionRejected`), and one observation/outcome pair for admission (plus, when burst ships, the `skipped-lease-loss` terminal outcome). `MaxDetached` positive is required under `DispatchDeferred`: existing deferred configurations built without `DefaultRuntimeOptions` must set it. Deliberate — no unbounded production default survives this ADR.
