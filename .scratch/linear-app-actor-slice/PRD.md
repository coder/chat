...
- Accept Linear **Agent Session Event** actions that represent session creation, including Linear-created assignment/delegation sessions, and user prompting. Other Linear webhook types and actions are ignored in MVP.
...
- Add delegation/session-created tests proving Linear-created assignment/delegation `AgentSessionEvent` payloads route through `OnNewMention`.
